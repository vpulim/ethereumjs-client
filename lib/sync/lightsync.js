...
class LightSynchronizer extends Synchronizer {
   ...
   * synchronize to this peer's headerchain. Returns a promise that resolves once
   * an origin peer is found.
   * @return {Object} { peer, height }
   */
  origin () {
    let origin
    const peers = this.pool.peers.filter(p => p.les && p.les.status.serveHeaders)
    if (!peers.length) return
    for (let peer of peers) {
      const td = peer.les.status.headTd
      if ((!origin && td.gte(this.chain.headers.td)) ||
          (origin && origin.les.status.headTd.lt(td))) {
        origin = peer
      }
    }
    if (origin) {
      return { peer: origin, height: origin.les.status.headNum }
    }
  }
