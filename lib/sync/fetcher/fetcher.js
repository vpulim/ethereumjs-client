...
class Fetcher extends Readable {
  ...
  startPipe () {
    if (typeof this.write !== 'function') return

    const fetcher = this
    const writer = new Writable({
      async write (data, encoding, cb) {
        try {
          await fetcher.write(data)
          fetcher.emit('write', data)
          cb()
        } catch (error) {
          cb(error)
        }
      ...
      writev (many, cb) {
        this.write([].concat(...many.map(x => x.chunk)), null, cb)
      }
    })
    this
      .pipe(writer)
  ...
  /**
   * Returns a writer for piping fetch results to. Should return null if fetcher
   * should not pipe results to a writer.
   * @return {Writable}
   */
  writer () {
    return null
